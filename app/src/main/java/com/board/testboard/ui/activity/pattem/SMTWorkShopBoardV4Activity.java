...
/**
 * 备注：修改了制令单产出方法【GSD还未启用新的制令单产出相关表，】
 ...
 */
public class SMTWorkShopBoardV4Activity extends BasePattemV4WorkshopActivity {
    ...
    private final String TAG = SMTWorkShopBoardV4Activity.class.getSimpleName();
    //颜色信息
    ...
    private long nSMTTotalCurRefreshTime = 0l;//当前时间
    ...
    private boolean IsTimeTask = false; //判断是否启用计时器
    private Timer timer;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context = SMTWorkShopBoardV4Activity.this;

    }

    @Override
    protected int getModuleLayoutId() {
        return R.layout.activity_smt_work_shop_board_v4;
    }
    ...
    private void initTitle() {
        ...
        menuTitle.setBtnRightOnclickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                openActivity(context, SettingSMTWorkShopBoardActivity.class,"Activity","SMTWorkShopBoardV4Activity");
            }
        });
    }
...
//        viewLineProduction.setBackgroundColor(color);//线别产能
    ...
    public void getPattem(String sKanBanMCNo) {
        getKanBanUserPattem(context, sKanBanMCNo, new OnResultListener() {
            ...
            public void onSuccess(String sDateNow) {
                ...
                mc_pattem = dbUtil.getMcPattemByCode(template_code[8]);//更新展示模板
                user_pattem = dbUtil.getUserPattemByCode(template_code[8]);//更新展示模板
                if (user_pattem != null) {
                    ...
                    SharedPreferencesUtil.saveValue(context, "nSMTTotalMcPattemLastRefreshTimeV4", DateUtils.dateToLong(rTime));
    ...
    private void init() {
        ...
        if (TextUtils.isEmpty(sLineNoIn)) {
            ...
            toast  = Toast.makeText(context,context.getString(R.string.more_line_select_line),Toast.LENGTH_SHORT);
            toast.show();
        ...
        if(TextUtils.isEmpty(sStationNoIn)){
            ...
            toast  = Toast.makeText(context,context.getString(R.string.please_select_station),Toast.LENGTH_SHORT);
            toast.show();
        ...
        smtTotalAlarmReceiver.setOnReceivedMessageListener(new SMTTotalAlarmReceiver.OnAlarmMessageListener() {
            ...
            public void onReceived(String message) {
                if (isStopReceiver) {
                    DBUtil dbUtil = new DBUtil(context);
                    if (isAutoPattem.equals("Y")) {
                        nSMTTotalMcPattemLastRefreshTime = SharedPreferencesUtil.getValue(context, "nSMTTotalMcPattemLastRefreshTimeV4", 0l);
                        ...
                        if (timeRefresh) {
                            ...
                            if (tempPattem != null) {
                                ...
                                if (!TextUtils.isEmpty(className)) {
                                    if (!className.equals("com.topcee.kanban.ui.activity.pattem.SMTWorkShopBoardV4Activity")) {
                                        openActivity(context, className, "isAutoPattem", isAutoPattem);
                                        SMTWorkShopBoardV4Activity.this.finish();
                                    } else {
                                        initData();
                                    }
                                    return;
    ...
    private void initData() {
        nSMTTotalWoLastRefreshTime = SharedPreferencesUtil.getValue(context, "nSMTTotalWoLastRefreshTimeV4", 0l);
        nSMTTotalWoProductionLastRefreshTime = SharedPreferencesUtil.getValue(context, "nSMTTotalWoProductionLastRefreshTimeV4", 0l);
        nSMTTotalLineStatusLastRefreshTime = SharedPreferencesUtil.getValue(context, "nSMTTotalLineStatusLastRefreshTimeV4", 0l);
//        nSMTTotalLineProductionLastRefreshTime = SharedPreferencesUtil.getValue(context, "nSMTTotalLineProductionLastRefreshTimeV4", 0l);
        nSMTTotalShiftLastRefreshTime = SharedPreferencesUtil.getValue(context, "nSMTTotalShiftLastRefreshTimeV4", 0l);
        nSMTTotalMessageLastRefreshTime = SharedPreferencesUtil.getValue(context, "nSMTTotalMessageLastRefreshTimeV4", 0l);
        nSMTTotalStationGoodRateLastRefreshTime = SharedPreferencesUtil.getValue(context, "nSMTTotalStationGoodRateLastRefreshTimeV4", 0l);
        ...
        LogUtil.e(TAG,"sCurLineNoIn = " + sCurLineNoIn);
        ...
        if (blIsInitData || (woTimeRefresh && woProductionTimeRefresh && lineStatusTimeRefresh && rateTimeRefresh)) {
            ...
            setPageText(String.format(getString(R.string.smt_total_show_page),iCurrentPage,nCount,inList_size));//当前第iCurrentPage页，共nCount页，共inList_size行
            getTimsGetWoInfo("SMT", "");
            getWorkStatus("SMT");
            getWoProductionInfo("SMT");

            long lastListItemRefreshTime = SharedPreferencesUtil.getValue(getBaseContext(), "lastListItemRefreshTimeV4", 0l);
            ...
            if( listTimeRefresh){
                LogUtil.e("nId ===== " + nId);
                nId++;
                SharedPreferencesUtil.saveValue(getBaseContext(), "lastListItemRefreshTimeV4", System.currentTimeMillis());
            }
            if(IsGetData){
                IsGetData = false;
                nId = 0; //相当于当前页号-初始化变量
        ...
        getTimsGetWoInfo(sInterfaceType, sCurLineNoIn, sWorkShopNo, sSMTWorkshopLanguage, new OnGetDataListener() {
            ...
            public void onGetData(String result) {
                ...
                try {
                    ...
                    SharedPreferencesUtil.saveValue(context, "nSMTTotalWoLastRefreshTimeV4", DateUtils.dateToLong(sDateNow));
...
//                            LogUtil.e(TAG, "wo===woBeanList = " + woBeanList.toString());
                    ...
                    LogUtil.e(TAG, "wo===sCurLineNoIn = " + sCurLineNoIn);
...
//                            LogUtil.e(TAG, "wo===woProductionBeanList = " + woProductionBeanList.toString());
...
//                            LogUtil.e(TAG, "wo===lineWorkStatusBeanList = " + lineWorkStatusBeanList.toString());
...
//                            LogUtil.e(TAG, "wo===yieldBeans = " + yieldBeans.toString());
                    ...
                    if (workshopTableBeans != null) {
                        LogUtil.e(TAG, "wo===workshopTableBeans = " + workshopTableBeans.toString());
    ...
    public void getWoProductionInfo(String sInterfaceType) {
        getWoProductionInfoForWo(sInterfaceType, sCurLineNoIn, sSMTWorkshopLanguage, new OnGetDataListener() {
            @Override
            public void onGetData(String result) {
                if (TextUtils.isEmpty(result)) {
                    if(toast!=null){
                ...
                try {
                    ...
                    SharedPreferencesUtil.saveValue(context, "nSMTTotalWoProductionLastRefreshTimeV4", DateUtils.dateToLong(sDateNow));
                    String rTime = sDateNow;
                    if (!TextUtils.isEmpty(time)) {
                        switch (time) {
                            case "refreshTime":
                    ...
                    for (int i = 0; i < sLineNoTemp.length; i++) {
                        ...
                        workshopTableBean.setLineNo(sLineNoTemp[i]);
                        if (woProductionBeanList != null && woProductionBeanList.size() >= 1) {
//                            LogUtil.e(TAG, "woproduct===woProductionBeanList = " + woProductionBeanList.toString());
                            for (int j = 0; j < woProductionBeanList.size(); j++) {
                                WoProductionBean woProductionBean = woProductionBeanList.get(j);
                                if(sLineNoTemp[i].equals(woProductionBean.getsLineNo())){
                                    workshopTableBean.setWo(woProductionBean.getsWo());
                        ...
                        if (lineWorkStatusBeanList != null && lineWorkStatusBeanList.size() >= 1) {
//                            LogUtil.e(TAG, "woproduct===lineWorkStatusBeanList = " + lineWorkStatusBeanList.toString());
                            for (int j = 0; j < lineWorkStatusBeanList.size(); j++) {
                                if (sLineNoTemp[i].equals(lineWorkStatusBeanList.get(j).getsLineNo())) {
                                    workshopTableBean.setWorkStatusNo(lineWorkStatusBeanList.get(j).getsWorkStatus());
                                    workshopTableBean.setWorkStatusName(lineWorkStatusBeanList.get(j).getsWorkStatusName());
                        ...
                        if (woBeanList != null && woBeanList.size() >= 1) {
//                            LogUtil.e(TAG, "woproduct===woBeanList = " + woBeanList.toString());
                            for (int j = 0; j < woBeanList.size(); j++) {
                        ...
                        if (yieldBeans != null && yieldBeans.size() >= 1) {
//                            LogUtil.e(TAG, "woproduct===yieldBeans = " + yieldBeans.toString());
                            for (int j = 0; j < yieldBeans.size(); j++) {
                                if (sLineNoTemp[i].equals(yieldBeans.get(j).getsLineNo())) {
                                    workshopTableBean.setGoodRate(yieldBeans.get(j).getsOutputYield());
                                }
                    ...
                    if (workshopTableBeans != null) {
                        LogUtil.e(TAG, "woproduct===workshopTableBeans = " + workshopTableBeans.toString());
                        viewTable.initTableView(context,workshopTableBeans);
                    }
        ...
        getWorkStatus(sInterfaceType, sCurLineNoIn, sSMTWorkshopLanguage, new OnGetDataListener() {
            ...
            public void onGetData(String result) {
                ...
                try {
                    ...
                    SharedPreferencesUtil.saveValue(context, "nSMTTotalLineStatusLastRefreshTimeV4", DateUtils.dateToLong(sDateNow));
                    String rTime = sDateNow;
                    if (!TextUtils.isEmpty(time)) {
                        switch (time) {
                            case "refreshTime":
                    ...
                    for (int i = 0; i < sLineNoTemp.length; i++) {
                        ...
                        workshopTableBean.setLineNo(sLineNoTemp[i]);
                        if (lineWorkStatusBeanList != null && lineWorkStatusBeanList.size() >= 1) {
//                            LogUtil.e(TAG, "lineworks===lineWorkStatusBeanList = " + lineWorkStatusBeanList.toString());
                            for (int j = 0; j < lineWorkStatusBeanList.size(); j++) {
                        ...
                        if (woProductionBeanList != null && woProductionBeanList.size() >= 1) {
//                            LogUtil.e(TAG, "lineworks===woProductionBeanList = " + woProductionBeanList.toString());
                            for (int j = 0; j < woProductionBeanList.size(); j++) {
                                WoProductionBean woProductionBean = woProductionBeanList.get(j);
                                if (sLineNoTemp[i].equals(woProductionBean.getsLineNo())) {
                                    workshopTableBean.setWo(woProductionBean.getsWo());
                        ...
                        if (woBeanList != null && woBeanList.size() >= 1) {
//                            LogUtil.e(TAG, "lineworks===woBeanList = " + woBeanList.toString());
                            for (int j = 0; j < woBeanList.size(); j++) {
                                if (sLineNoTemp[i].equals(woBeanList.get(j).getsLineNo())) {
                                    workshopTableBean.setCustomerNo(woBeanList.get(j).getsCustomerName());
                                    workshopTableBean.setWoPlanQty(woBeanList.get(j).getsWoQtyPlan());
                        ...
                        if (yieldBeans != null && yieldBeans.size() >= 1) {
//                            LogUtil.e(TAG, "lineworks===yieldBeans = " + yieldBeans.toString());
                            for (int j = 0; j < yieldBeans.size(); j++) {
                                if (sLineNoTemp[i].equals(yieldBeans.get(j).getsLineNo())) {
                                    workshopTableBean.setGoodRate(yieldBeans.get(j).getsOutputYield());
                                }
                    ...
                    if (workshopTableBeans != null) {
                        LogUtil.e(TAG, "lineworks===workshopTableBeans = " + workshopTableBeans.toString());
                        viewTable.initTableView(context,workshopTableBeans);
                    }
        ...
        getStationOutputYield(sWoIn, sStationNoIn, sSMTWorkshopLanguage, new OnGetDataListener() {
            ...
            public void onGetData(String result) {
                ...
                try {
                    ...
                    SharedPreferencesUtil.saveValue(context, "nSMTTotalStationGoodRateLastRefreshTimeV4", DateUtils.dateToLong(sDateNow));
                    String rTime = sDateNow;
                    if (!TextUtils.isEmpty(time)) {
                        switch (time) {
                            case "refreshTime":
                    ...
                    for (int i = 0; i < sLineNoTemp.length; i++) {
                        ...
                        workshopTableBean.setLineNo(sLineNoTemp[i]);
                        if (yieldBeans != null && yieldBeans.size() >= 1) {
//                            LogUtil.e(TAG, "yieldBeans===yieldBeans = " + yieldBeans.toString());
                            for (int j = 0; j < yieldBeans.size(); j++) {
                        ...
                        if (woProductionBeanList != null && woProductionBeanList.size() >= 1) {
//                            LogUtil.e(TAG, "yieldBeans===woProductionBeanList = " + woProductionBeanList.toString());
                            for (int j = 0; j < woProductionBeanList.size(); j++) {
                                WoProductionBean woProductionBean = woProductionBeanList.get(j);
                                if (sLineNoTemp[i].equals(woProductionBean.getsLineNo())) {
                                    workshopTableBean.setWo(woProductionBean.getsWo());
                        ...
                        if (woBeanList != null && woBeanList.size() >= 1) {
//                            LogUtil.e(TAG, "yieldBeans===woBeanList = " + woBeanList.toString());
                            for (int j = 0; j < woBeanList.size(); j++) {
                                if (sLineNoTemp[i].equals(woBeanList.get(j).getsLineNo())) {
                                    workshopTableBean.setCustomerNo(woBeanList.get(j).getsCustomerName());
                                    workshopTableBean.setWoPlanQty(woBeanList.get(j).getsWoQtyPlan());
                        ...
                        if (lineWorkStatusBeanList != null && lineWorkStatusBeanList.size() >= 1) {
//                            LogUtil.e(TAG, "yieldBeans===lineWorkStatusBeanList = " + lineWorkStatusBeanList.toString());
                            for (int j = 0; j < lineWorkStatusBeanList.size(); j++) {
                                if (sLineNoTemp[i].equals(lineWorkStatusBeanList.get(j).getsLineNo())) {
                                    workshopTableBean.setWorkStatusNo(lineWorkStatusBeanList.get(j).getsWorkStatus());
                                    workshopTableBean.setWorkStatusName(lineWorkStatusBeanList.get(j).getsWorkStatusName());
                    ...
                    if (workshopTableBeans != null) {
                        LogUtil.e(TAG, "yieldBeans===workshopTableBeans = " + workshopTableBeans.toString());
                        viewTable.initTableView(context,workshopTableBeans);
                    }
        ...
        getKanBanMsg(code, json, sSMTWorkshopLanguage, new OnGetDataListener() {
            ...
            public void onGetData(String result) {
                ...
                try {
                    ...
                    SharedPreferencesUtil.saveValue(context, "nSMTTotalMessageLastRefreshTimeV4", DateUtils.dateToLong(sDateNow));
                    String rTime = sDateNow;
                    if (!TextUtils.isEmpty(time)) {
                        switch (time) {
                            case "refreshTime":
        ...
        getShiftInfo(sInterfaceType, sLineNo, sSMTWorkshopLanguage, new OnGetDataListener() {
            ...
            public void onGetData(String result) {
                ...
                try {
                    ...
                    SharedPreferencesUtil.saveValue(context, "nSMTTotalShiftLastRefreshTimeV4", DateUtils.dateToLong(sDateNow));
                    String rTime = sDateNow;
                    if (!TextUtils.isEmpty(time)) {
                        switch (time) {
                            case "refreshTime":
...
//                            setShift(String.format(getString(R.string.smt_line_board_shift_no), sShiftName));//by modify 19/02/15
...
//                    //获两个日期时间的差值(sDbDate - localDate = 秒)
//                    String sSeconds = DateUtils.getDateTimeDifferences(localDate, sDateNow);
//                    long nSeconds = 0;
//                    if(TextUtils.isEmpty(sSeconds)){
//                        sSeconds = "0";
//                    }
//                    nSeconds = Long.parseLong(sSeconds);
//                    if(MyApplication.nSecondsDifferences >= 0){
//                        nSeconds = nSeconds + MyApplication.nSecondsDifferences; //处理同步服务器时间
//                    } else if(MyApplication.nSecondsDifferences < 0){
//                        nSeconds = nSeconds - MyApplication.nSecondsDifferences; //处理同步服务器时间
//                    }
//                    if(nSeconds < 0){
//                        nSeconds = 0;
...
//                    calendar.add (Calendar.SECOND, (int) nSeconds);
//                    String curTime = DateUtils.DateToString(calendar.getTime());
    ...
    protected void onPause() {
        ...
        stopScroll();//停止公告消息滚动
        isStopReceiver = false;//停止刷新数据
        stopHandler();//释放handler消息
        sStopService();//停止定时器启动
        stopTimer();//停止定时器启动
        stopToast();//停止弹出消息
    }
